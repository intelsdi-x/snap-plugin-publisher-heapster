...
func (p *processingContext) ingestContainerSpec(dockerID string, container *cadv.ContainerInfo, dtree map[string]interface{}) {
	var metricErr error
	vf := newValueFilter(&metricErr)
	vf.enter(fmt.Sprintf("%s/spec", dockerID))
	spec := &container.Spec
	if specRawObj, gotSpec := dtree["spec"]; gotSpec {
	...
	if memStatsRawObj, err := jsonutil.NewObjWalker(dtree).Seek("/cgroups/memory_stats/stats"); err == nil {
		spec.HasMemory = true
...
func (p *processingContext) updateContainerStats(container *cadv.ContainerInfo, metrics map[string]interface{}) {
	...
	statsObj, err := objWalker.Seek("/stats")
	if err != nil {
		return
	}
...
func (p *processingContext) ingestCPUStats(container *cadv.ContainerInfo, stats *cadv.ContainerStats, statsMap map[string]interface{}) {
	...
	usageObj, _ := jsonutil.NewObjWalker(statsMap).Seek("/cgroups/cpu_stats/cpu_usage")
	if usageObj != nil {
	...
	perCPUObj, _ := jsonutil.NewObjWalker(statsMap).Seek("/cgroups/cpu_stats/cpu_usage/percpu_usage")
...
func (p *processingContext) ingestMemoryStats(container *cadv.ContainerInfo, stats *cadv.ContainerStats, statsMap map[string]interface{}) {
	...
	memStatsObj, _ := jsonutil.NewObjWalker(statsMap).Seek("/cgroups/memory_stats/stats")
	memUsageObj, _ := jsonutil.NewObjWalker(statsMap).Seek("/cgroups/memory_stats/usage")
...
func (p *processingContext) ingestNetworkStats(container *cadv.ContainerInfo, stats *cadv.ContainerStats, statsMap map[string]interface{}) {
	...
	networksObj, _ := jsonutil.NewObjWalker(statsMap).Seek("/network")
	if networksObj == nil {
		return
	}
	...
	stats.Network.InterfaceStats = ifaceMap["total"]
...
func (p *processingContext) ingestConnectionStats(container *cadv.ContainerInfo, stats *cadv.ContainerStats, statsMap map[string]interface{}) {
	...
	tcpObj, _ := jsonutil.NewObjWalker(statsMap).Seek("/connection/tcp")
	if tcpObj != nil {
	...
	tcp6Obj, _ := jsonutil.NewObjWalker(statsMap).Seek("/connection/tcp6")
	if tcp6Obj != nil {
...
func (p *processingContext) ingestFsStats(container *cadv.ContainerInfo, stats *cadv.ContainerStats, statsMap map[string]interface{}) {
	...
	fsObj, _ := jsonutil.NewObjWalker(statsMap).Seek("/filesystem")
	if fsObj == nil {
		return
	}
...
func (m MetricMap) GetTimeFromStr(key string, defValue time.Time) (res time.Time, _ error) {
	if v, gotValue := m.Get(key); gotValue {
		if vstr, gotStr := v.(string); gotStr {
			return ParseTime(vstr)
		}
		return res, fmt.Errorf("can't handle metric map entry '%s' as string input for time parser", key)
	}
	return defValue, nil
}
...
// error variable.
func newValueFilter(outputErr *error) *valueFilter {
	res := valueFilter{register: outputErr}
	return &res
}
...
func (v *valueFilter) filter1(value interface{}, err error) interface{} {
	if err != nil && *v.register == nil {
		if v.context != "" {
			outputErr := fmt.Errorf("%s: %v", v.context, err)
			v.register = &outputErr
		} else {
			v.register = &err
		}
	}
