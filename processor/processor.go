...
func (p *processingContext) processMetrics(metrics jsonutil.MetricList) {
	...
	if dtree, err := jsonutil.NewObjWalker(mtree).Seek(dockerMetricPrefix); err == nil {
		...
		containerIds = p.ingestDockersTree(dtree.(map[string]interface{}))
	}
	p.ingestCustomMetrics(mtree)
	if len(containerIds) > 0 {
		for _, id := range containerIds {
			p.mergeCustomMetricsFor(id)
			p.discardTooOldCustomValuesFor(id)
		}
	}
...
func (p *processingContext) ingestDockersTree(dtree map[string]interface{}) (containerIds []string) {
...
func (p *processingContext) ingestCPUStats(container *cadv.ContainerInfo, stats *cadv.ContainerStats, statsMap map[string]interface{}) {
	...
	if usageObj := vf.filter1(jsonutil.NewObjWalker(statsMap).Seek("/cgroups/cpu_stats/cpu_usage")); usageObj != nil {
