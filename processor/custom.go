...
func init() {
	...
	exchange.LoggerControl.WireLogger(clog)
}

// Function  mergeCustomMetricsFor flushes pending custom metric values into
...
// Function  discardTooOldCustomValuesFor removes pending custom metric values
...
// Function  ingestCustomMetrics scans tree of metrics for items that are
//tagged as custom.
...
func (p *processingContext) ingestCustomMetrics(mtree map[string]interface{}) {
	util.NewObjWalker(mtree).Walk("/", func(path string, info os.FileInfo, err error) error {
...
// Function  ingestCustomMetricsFrom extracts custom metric Spec item(s) basing
...
// Function  insertIntoCustomMetrics inserts custom metric specs into
...
func (p *processingContext) insertIntoCustomMetrics(containerPath string, container *cadv.ContainerInfo, specs []cadv.MetricSpec, metric *plugin.MetricType) {
	...
	if num_extracted > 0 {
		clog.WithFields(logrus.Fields{"num_extracted": num_extracted, "value_names": dbg_valuesIn}).Debug("extracted custom metrics")
	}
}

// Function  extractCustomMetrics extracts custom metric Spec item(s) basing
//on information associated with given metric instance.
...
func (p *processingContext) extractCustomMetrics(metric *plugin.MetricType) (containerPath string, specs []cadv.MetricSpec, valid bool) {
	...
	if valueMap, isMap := metric.Data().(map[string]float64); !isMap {
		...
		return containerPath1, specs, true
	} else {
...
// Function  extractOneCustomMetric extracts one custom metric Spec basing on
...
// Function  extractCustomValues extracts custom metric Values basing on metric
...
// Function  extractOneCustomValue extracts a single custom metric Value basing
